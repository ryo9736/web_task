...
begin
  ...
  result１ = connection.exec("select date, weight, give_for from crops
                             where not(give_for = '自家消費');")
  result２ = connection.exec("select date, weight, give_for from crops
                             where quality = false ;")
  # 取り出した各行を処理する
  result１.each do |record|
      # 各行を取り出し、putsでターミナル上に出力する
      puts "ゴーヤの大きさ：#{record["weight"]}　売った相手：#{record["give_for"]
      }"
  end

  result２.each do |record|
    # 各行を取り出し、putsでターミナル上に出力する
    puts "ゴーヤの大きさ：#{record["weight"]}　売った相手：#{record["give_for"]}"
end
